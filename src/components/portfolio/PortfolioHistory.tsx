...
export function PortfolioHistory() {
  ...
  const loadSnapshots = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('portfolio_snapshots').select('*').order('created_at', {
        ascending: false
      });
  ...
  const downloadSnapshot = (snapshot: PortfolioSnapshot) => {
    ...
    // Summary sheet
    const summaryData = [{
      Metric: 'Total Value (USD)',
      Value: snapshot.total_value_usd
    }, {
      Metric: 'Private Equity (USD)',
      Value: snapshot.private_equity_value_usd
    }, {
      Metric: 'Public Equity (USD)',
      Value: snapshot.public_equity_value_usd
    }, {
      Metric: 'Fixed Income (USD)',
      Value: snapshot.fixed_income_value_usd
    }, {
  ...
  return <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {snapshots.map(snapshot => <Card key={snapshot.id} className="hover:shadow-md transition-shadow">
            <CardHeader className="pb-3">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => deleteSnapshot(snapshot)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                          Delete
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
              ...
              {snapshot.description && <p className="text-sm text-muted-foreground mt-2">
