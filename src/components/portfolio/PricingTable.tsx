...
export function PricingTable({
  ...
  const consolidatedAssets = useMemo(() => {
    ...
    return Array.from(assetMap.entries()).map(([name, assets]) => {
      const mostRecent = assets.reduce((latest, current) => new Date(current.updated_at) > new Date(latest.updated_at) ? current : latest);
      return {
        ...mostRecent,
  ...
  const SortIcon = ({
    ...
    column: "name" | "sub_class" | "updated_at";
  }) => {
    if (sortBy !== column) {
  ...
  const handleStartEdit = (asset: Asset, field?: "price" | "ytw") => {
    setEditingAsset({
      ...
      ytw: asset.ytw ? (asset.ytw * 100).toString() : "",
      // Convert to percentage for editing
      field
    });
  };
  ...
  const isGroupB = (asset: Asset) => {
    return asset.class === "Fixed Income" && ["Corporate", "Gov long", "Gov 1-2", "CPI linked"].includes(asset.sub_class);
  };
  ...
  return <Card>
      ...
      <CardContent>
        <div className="rounded-md border max-h-[600px] overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-muted-foreground border-b z-10">
              <tr className="text-left">
                <th className="p-2 font-medium text-muted cursor-pointer hover:text-foreground transition-colors select-none" onClick={() => handleSort("name")}>
                  <div className="flex items-center gap-1">
                    Asset Name
                    <SortIcon column="name" />
                  </div>
                </th>
                <th className="p-2 font-medium text-muted cursor-pointer hover:text-foreground transition-colors select-none" onClick={() => handleSort("sub_class")}>
                  <div className="flex items-center gap-1">
                    Sub Class
                ...
                <th className="p-2 font-medium text-muted cursor-pointer hover:text-foreground transition-colors select-none" onClick={() => handleSort("updated_at")}>
                  <div className="flex items-center gap-1">
                    Last Updated
            ...
            <tbody>
              ...
              return <tr key={asset.id} className={`hover:bg-chart-8/50 transition-colors ${isEven ? "bg-muted/20" : "bg-background"}`}>
                    ...
                    <td className="p-2 py-1.5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {isEditing ? <>
                            <Input ref={priceInputRef} type="text" inputMode="decimal" value={editingAsset.price} onChange={e => setEditingAsset(prev => prev ? {
                        ...prev,
                        price: validateNumericInput(e.target.value)
                      } : null)} onKeyDown={handleKeyDown} className="w-20 h-7 text-xs text-right" />
                            <span className="text-xs text-muted-foreground min-w-[28px]">{asset.origin_currency}</span>
                          </> : <>
                            <span className="font-medium cursor-pointer hover:bg-muted/50 px-1 py-0.5 rounded transition-colors" onDoubleClick={() => handleDoubleClick(asset, "price")} title="Double-click to edit">
                              {asset.price}
                            </span>
                    ...
                    <td className="p-2 py-1.5 text-right">
                      {showYTW ? <div className="flex items-center justify-end gap-1">
                          {isEditing ? <>
                              <Input ref={ytwInputRef} type="text" inputMode="decimal" value={editingAsset.ytw} onChange={e => setEditingAsset(prev => prev ? {
                        ...prev,
                        ytw: validateNumericInput(e.target.value)
                      } : null)} onKeyDown={handleKeyDown} className="w-16 h-7 text-xs text-right" placeholder="0.00" />
                              <span className="text-xs text-muted-foreground">%</span>
                            </> : <>
                              <span className="font-medium cursor-pointer hover:bg-muted/50 px-1 py-0.5 rounded transition-colors" onDoubleClick={() => handleDoubleClick(asset, "ytw")} title="Double-click to edit">
                    ...
                    <td className="p-2 py-1.5">
                      <div className="flex items-center gap-1">
                        {isEditing ? <>
                            <Button size="sm" variant="ghost" onClick={handleSave} className="h-6 w-6 p-0 text-financial-success hover:text-financial-success hover:bg-financial-success/10">
                              <Check className="h-3 w-3" />
                            </Button>
                            <Button size="sm" variant="ghost" onClick={handleCancel} className="h-6 w-6 p-0 text-muted-foreground hover:text-destructive hover:bg-destructive/10">
                              <X className="h-3 w-3" />
                            </Button>
                          </> : <Button size="sm" variant="ghost" onClick={() => handleStartEdit(asset)} className="h-6 w-6 p-0 text-muted-foreground hover:text-foreground hover:bg-muted">
          ...
          {sortedAssets.length === 0 && <div className="p-8 text-center text-muted-foreground">
