...
export function PendingAssets({
  ...
  const handleCreateAsset = (asset: PendingAsset) => {
    onCreateAsset({
      name: asset.name,
      class: asset.asset_class as AssetClass
    });
  };
  ...
  if (isLoading) {
    return <Card className="bg-gradient-to-br from-muted/30 to-muted/10 shadow-card border-border/50">
        <CardHeader>
  ...
  return <>
      <Card className="bg-gradient-to-br from-muted/30 to-muted/10 shadow-card border-border/50">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-lg font-normal text-zinc-600">
        ...
        <CardContent>
          {pendingAssets.length === 0 ? <div className="text-center py-6 text-muted-foreground">
              <p className="text-sm">No pending assets</p>
              <Button variant="ghost" size="sm" onClick={() => setIsDialogOpen(true)} className="mt-2 gap-1">
              ...
              <TableBody>
                {pendingAssets.map(asset => <TableRow key={asset.id}>
                    ...
                    if (count > 0) {
                      return <Badge className="text-xs px-1.5 py-0 bg-primary/20 text-primary border-primary/30">
                    ...
                    <TableCell className="py-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleCreateAsset(asset)} className="gap-2">
                            <PlusCircle className="h-4 w-4" />
                            Create Asset
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleRemove(asset.id)} className="gap-2 text-destructive focus:text-destructive">
      ...
      <PendingAssetDialog isOpen={isDialogOpen} onClose={handleDialogClose} onSave={handleAddPendingAsset} onUpdate={handleUpdatePendingAsset} existingAssets={existingAssets} editingAsset={editingAsset} />
    </>;
}